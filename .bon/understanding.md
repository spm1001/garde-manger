...
> **Status: DECOMMISSIONED 2026-06-03.** garde-manger is fully retired (all 7 gm-kudasu gates done) and this repo is archived on GitHub. Its function — clean finished sessions → keep → find — folds into `~/notes`. This document is now a *teardown map*, not a portrait of a living system. The plan is **gm-kudasu** + 7 ordered actions; **gm-gumopi** (format), **gm-firaso** (predicate + AMP + tier) **gm-kenave** (live hook — built, published as bon 0.23.0, **verified live**) and **gm-jewode** (the bulk parse — **done 2026-06-03**: 1,763 unique sessions in `notes/raw/claude/code/`) are done; **gm-jerapu** (verified complete + grep-proven on both machines), **gm-damoku** (consumers repointed/retired — nothing calls garde) and **gm-pamadu** (engines off; DB cold-backed-up to `~/.claude/memory/backups/memory.db.decommission-2026-06-03.gz` = 9,940 sources / 7,613 extractions / 9,919 summaries, integrity-verified; repo archived) are likewise complete. **Final corpus: 1,763 unique sessions / 112 MB** (1,505 Hezza + 216 Mac-only + 42 amp). NB the "106-session ark" that drove the original DB-deletion caution was **debunked 2026-06-03** (a matching artifact — see risk #1): the real off-both-disks set is 3 trivial sessions, so a cold DB backup is prudent hygiene, not the high-stakes gate it was framed as.

## The decision, in one breath

garde-the-database is over-built for its actual job. The job was only ever "clean finished sessions and make them findable." Measuring the payload collapsed the edifice: deglacer compresses raw CC JSONL ~74×, and the real corpus is **1,721 unique substantial CC sessions + 42 amp threads = 1,763 files / 112 MB on disk** (`gm-jewode`, 2026-06-03). gm-firaso's full deglacer pass over 5,948 non-agent sessions matched ~2,267 keep-*decisions* — but those were **file-appearances**: ~761 were the same long sessions resumed across multiple project dirs, collapsed one-file-per-session by `skip:exists` (the same count-with-the-wrong-noun shape as the debunked ark). The 112 MB on disk > gm-firaso's 60.8 MB conv-only estimate because written bodies include tools + thinking. Add 42 amp threads (~33 MB, Source C); the "~106 DB-only orphans" were a miscount (risk #1 — only 3 trivial sessions are genuinely DB-only). The 692 MB DB is mostly FTS-index bloat over that. deglacer is now a shared library, so it survives garde. `~/notes` already solves cross-machine (Syncthing) and is building a raw→digest→promote pipeline (`notes-sobute`) whose cleaning step *is* deglacer.

So: **retire garde-the-DB, keep garde-the-idea inside notes.** The corpus folds into `notes/raw/`. Search becomes Claude+grep over that folder — at ~112 MB / 1,763 files, a Claude with grep out-searches BM25 because it *judges* relevance rather than just ranking it. No database needed.
...
| 3 | **gm-jewode** ✓ | **DONE 2026-06-03.** Built the canonical historical corpus into `notes/raw/claude/code/`: **1,763 files** = 1,505 Hezza (Source A) + 216 Mac-only (the 295 delta, converted on Hezza since the Mac lacks deglacer) + 42 amp (Source C). Source B dropped (no ark — risk #1). 0 integrity failures. | The actual preservation; depends on the format + capture rules above |
